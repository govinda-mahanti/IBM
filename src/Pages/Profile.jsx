    ...
    <div className="bg-gray-800 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-6">
      <img
        ...
        onError={(e) => {
          ...
          e.target.src = "https://placehold.co/128x128/7e22ce/ffffff?text=AD";
        }}
      />
        ...
        {isEditMode ? (
          <div className="space-y-3">
            <input
              ...
              className="w-full bg-gray-700 text-white text-2xl font-bold rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
            <input
              type="email"
              name="email"
              ...
              onChange={handleInputChange}
              className="w-full bg-gray-700 text-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
            />
            <input
              type="tel"
              name="phone"
...
const SessionHistory = () => {
  const getServiceIcon = (service) => {
    const iconContainerBaseClasses =
      "w-10 h-10 p-2 rounded-full text-white flex items-center justify-center";
    ...
    switch (service.toLowerCase()) {
      case "therapist":
        return (
          <div
            className={`${iconContainerBaseClasses} bg-gradient-to-br from-blue-500 to-indigo-600`}
          >
            <HeartHandshake {...iconProps} />
          </div>
        );
      case "yoga":
        return (
          <div
            className={`${iconContainerBaseClasses} bg-gradient-to-br from-green-400 to-teal-500`}
          >
            <Leaf {...iconProps} />
          </div>
        );
      case "psychiatrist":
        return (
          <div
            className={`${iconContainerBaseClasses} bg-gradient-to-br from-pink-500 to-rose-500`}
          >
      ...
      default:
        return (
          <div className={`${iconContainerBaseClasses} bg-gray-500`}>
            <User {...iconProps} />
          </div>
        );
    }
      ...
      <div className="space-y-4">
        {Object.keys(sessionCounts).length > 0 ? (
          Object.entries(sessionCounts).map(([service, count]) => (
            <div
              key={service}
              className="bg-gray-700/50 p-4 rounded-lg flex items-center justify-between gap-4"
            >
          ...
          >
            ...
            {plan.name === "Yearly" && (
              <p className="text-center text-xs mt-1">
                Billed as {plan.price.yearly}/year
              </p>
            )}
            ...
            <ul className="mt-6 space-y-3 flex-grow">
              {plan.features.map((feature) => (
                <li key={feature} className="flex items-start gap-3">
                  <div
                    className={`flex-shrink-0 w-5 h-5 rounded-full mt-1 ${
                      plan.highlight ? "bg-white/30" : "bg-purple-500"
            ...
            <button
              className={`w-full mt-8 py-2 px-4 rounded-lg font-semibold transition-colors ${
                plan.isCurrent
